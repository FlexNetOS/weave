...
| 2026-06-27 | Accepted the layered Cargo workspace as the supported architecture (ADR-0006 / WL-077) | CLAUDE.md, ARCHITECTURE.md, `.handoff/decisions/ADR-0006-…` | Re-evaluated WL-043 after token-light MCP, feature-gated surfaces/obscura, cross-machine delivery, and runner/job orchestration. The invariant is one dependency-light Rust binary, not one crate; the four crates are accepted load-bearing boundaries. WL-043 is closed/replaced; do not add new crates without ADR-level justification. |
...
Today the binary is assembled from a **small internal Cargo workspace** — `weave-core` ← `weave-inject` ← `weave-mcp` ← `weave` (bin), with no upward deps — that still links to **one** dependency-light static binary (`target/release/weave`). ADR-0006 accepts this layered workspace as the supported architecture: the invariant is **one Rust binary**, not one crate. Work within the four crates and do **not** add new crates without an ADR-level reason that preserves the dependency direction.
