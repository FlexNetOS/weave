...
impl LibsqlStore {
    pub fn open(cfg: &Config) -> Result<Self> {
        ...
        let (db, conn) = rt.block_on(async move {
            ...
            // Migration (WL-018): a DB created before birth certificates lacks the
            // `peers.birth_cert` column. Add it idempotently (mirrors SqliteStore::migrate).
            // Nullable TEXT; defaults to NULL for existing rows (== no cert yet), which
            // triggers a backward-compat mint on the next register_peer_full call.
            {
                let probe =
                    "SELECT 1 FROM pragma_table_info('peers') WHERE name='birth_cert'";
                let mut it = conn.query(probe, ()).await?;
                if it.next().await?.is_none() {
                    conn.execute("ALTER TABLE peers ADD COLUMN birth_cert TEXT", ())
                        .await
