...
| Hosted relay (`repowire.io`, outbound WSS) | consent-based cross-machine **PUSH** (ADR-0005) + cross-store federation (Tier-1) + Tier-2 pull | ✅ HAVE (daemon-free push, owner-only-writes) | WL-056: `weave push --to <name> --host <https-origin>` POSTs a signed Intent to B's dedicated `POST /push` route, enabled by `weave serve --push-token …` or `weave dashboard --push-token …` with a credential that must differ from the operator token (HTTP/plain host is loopback-only; redirects are refused). B commits into its OWN inbox via the SAME `commit_pulled` pipeline and lights its OWN pane WITHOUT polling. Daemon-free, owner-only-writes, verify-on-commit, default build byte-identical. `mcp::tool_push`, `http.rs` `--bind` fail-closed, `main.rs` `Cmd::Push` |
