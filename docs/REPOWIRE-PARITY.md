...
| Session export / resume (cross_agent_session_resumer / casr) | `weave session export` / `weave session import` | ✅ HAVE | **WL-040**; portable, schema-versioned JSON interchange (messages + mesh memory) across distinct instances; import reuses `Store::send` (free id-remap, idempotent dedup on idempotency_key, synth key `wl040:<id>:<n>` for keyless legacy); identity remap via `--as`; `--dry-run`; **no new Store method / schema / standing MCP tool**; asks recorded-not-replayed (→ WL-040b), peers excluded by design. See `FORMAT-session-export.md` |
