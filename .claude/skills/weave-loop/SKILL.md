---
description: Autonomous, resumable weave-loop — one backlog item per cycle, commits per cycle, hands off at the cycle budget via the session-relay skill. Replaces a single long session with a chain of short ones; cold resume from the committed .handoff/packets/latest.md. Use when the user wants to work through the M1/M3 roadmap (WL-001..WL-007) one item at a time, in a fresh git worktree.
metadata:
  type: loop
  owner: weave-harness
---
...
The committed `.handoff/packets/latest.md` is the **authoritative** resume signal; weave is the
observable cross-identity heartbeat. The external runner is
`.claude/skills/weave-loop/scripts/ralph-weave.sh`.

## Entry points

- **DISCOVER** — no `backlog.md` yet (or empty): seed it from `.handoff/loop/TASKS.md` M1/M3 and write
  `loop_state.md`. Stop after seeding.
- **RESUME** — `.handoff/packets/latest.md` present: run `verify-on-resume`, broadcast `relay:resumed`
...
2. Read `.handoff/loop/TASKS.md` M1/M3. Mirror the items into `.handoff/loop/backlog.md` as `WL-NNN` slugs with
...
## RESUME (cold start from `.handoff/packets/latest.md`)
...
2. `bash .handoff/loop/verify-on-resume.sh`. If it fails, write `NEEDS-HUMAN` with the captured
...
3. **Bootstrap hazard check:** if the last landed commit touches `mcp.rs` / `store.rs` /
   ...
   `.handoff/packets/latest.md` as the resume signal. Re-verify the heartbeat **after** the build passes
...
5. In `loop_state.md` set `cycles_this_session=0`, bump `cycles_total` by the carry from
   `.handoff/packets/latest.md` (if any), set `status: RESUMED — at item <NEXT>`, update `last_update`.
...
1. **Stop-checks (in this order):**
   - `touch .handoff/loop/STOP` exists → halt (kill switch).
...
4. **Run the weave-orchestrator pipeline (phases 1-3).**
   - **Phase 1 — Plan:** invoke `weave-planner`. It writes `.handoff/loop/01_planner_plan.md`.
   - **Phase 2 — Implement:** invoke `weave-implementer`. It edits `src/`, mirrors Store changes across both backends, confirms both compile. Writes `.handoff/loop/02_implementer_changes.md`.
   - **Phase 3 — Verify:** invoke `weave-verifier`. It adds matching test layers and runs the full gate on **both** backends (fmt, clippy `-D warnings`, test). Writes `.handoff/loop/03_verifier_report.md`.
...
5. **Guardian review + approve (Phase 4) — MiniMax is the external guardian.**
   Spawn MiniMax (`minimax-m3:cloud` via the configured guardian command) with the uncommitted diff, the plan, the change log, and the verifier report. MiniMax audits against `weave-invariants`, runs the `weave-drift-guard` scan, checks docs sync, and writes `.handoff/loop/04_guardian_review.md` with **APPROVE** or **BLOCK**.
...
2. Write `.handoff/loop/DONE` with the evidence:
   ```
   closed: <UTC>
   cycles_total: <N>
...
- **The committed `.handoff/packets/latest.md` is authoritative.** A same-machine successor inherits the
